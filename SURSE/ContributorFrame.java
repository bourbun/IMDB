...
public class ContributorFrame extends JFrame implements RequestsManager,StaffInterface,Observer{
	...
	public ContributorFrame(User user) {
		...
		String[] actions = new String[]{"View productions details", "View actors details", "View notifications",
				"Search for actor/production","Add/Delete a request","Add/Delete actor/production to/from favorites","Add/Delete actor/movie/series from system","Solve a request","Update Movie/Actor Details","See favorites"};
	...
	private void addOrDeleteRequest() {
		...
		addButton.addActionListener(e -> showRequestForm(dialog, RegularFrame.RequestAction.ADD));
		deleteButton.addActionListener(e -> showRequestForm(dialog, RegularFrame.RequestAction.DELETE));
...
// Panel for radio buttons
	...
	private void solveRequest() {
		...
		List<Request> userRequests = IMDB.getInstance().getRequestsForUser(user.name);
		DefaultListModel<String> listModel = new DefaultListModel<>();
		for (Request request : userRequests) {
